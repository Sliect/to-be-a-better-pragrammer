...
export function breathFirstSearch<T extends { _instance: boolean }>(
  list: T[],
  cb: (node: TreeNode<T>, parent: TreeNode<T>) => void,
  opts: Opts
) {
  const queue = [...list];
  ...
  while (queue.length) {
    ...
    const children = top?.[childrenKey as keyof typeof top] || [];
...
export class Tree {
  ...
  findSiblingsIncludeSelf(key: string) {
  ...
  /** 获取子孙节点 */
  findPosterityNodesByKey(key: string) {
    const result: any[] = [];
    const node = this.findNodeByKey(key);

    const traverse = (n: { [x: string]: any[] }) => {
  ...
  findIndex(key: string) {
    ...
    return arr.map((it: { [x: string]: any }) => it.key).indexOf(key);
  }

  /** 获取所有叶子节点 */
  findLeafNodesByKey(key: string) {
    const node = this.findNodeByKey(key);
  ...
  /** 获取第一层级的所有节点 */
  findFirstLevelNodes() {
    return this.filter((node) => node.parent?.data === root);
  }
  ...
  forEach(iteratee: (value: any, index: number, array: any[]) => void) {
  ...
  map(iteratee: (value: any, index: number, array: any[]) => unknown) {
...
class TreeNode<T extends { _instance?: any }> {
  data: T;
  private _key;
  ...
  get key() {
    return this.data[this._key as keyof typeof this.data];
  }
