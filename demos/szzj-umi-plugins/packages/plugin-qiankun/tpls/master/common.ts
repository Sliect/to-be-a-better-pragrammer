...
export function patchMicroAppRoute(
  ...
  if (microAppName) {
    if (route.children?.length) {
      const childrenRouteHasComponent = route.children.some((r: any) => r.element);
...
const recursiveSearch = (
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    if (routes[i].children && routes[i].children?.length) {
      const found = recursiveSearch(routes[i].children || [], path, routes[i].path);
      if (found) {
        return found;
      }
    }
...
export function insertRoute(routes: IRouteProps[], microAppRoute: IRouteProps) {
  const mod =
    microAppRoute.appendChildTo || microAppRoute.insert
      ...
      : undefined;
  const target =
    microAppRoute.appendChildTo || microAppRoute.insert || microAppRoute.insertBefore;
