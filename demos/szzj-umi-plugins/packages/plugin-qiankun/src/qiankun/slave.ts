...
function handleOriginalHtml(api: IApi, microAppEntry: string, originalHtml: string) {
  ...
  assert(appName, '[@umijs/plugin-qiankun]: You should have name in package.json');
...
if (!window.__POWERED_BY_QIANKUN__) {
  ...
  function getFileContent(file: string) {
    return readFileSync(join(__dirname, '../../tpls/slave', file), 'utf-8');
  }
      ...
      if (masterEntry && proxyToMasterEnabled) {
        ...
        return createProxyMiddleware((pathname) => pathname !== '/local-dev-server', {
          ...
          onProxyRes: responseInterceptor(
            async (responseBuffer: any, proxyRes: any, req: any, res: any) => {
