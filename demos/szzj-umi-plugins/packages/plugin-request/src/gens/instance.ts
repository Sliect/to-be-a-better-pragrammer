...
import { HttpStatusMap, errorHandler } from './utils';
import { EncryptMiddleware, DecryptMiddleware, AttachmentMiddleware } from './middlewares';
...
export const getInstance = (options: RequestOptionsInit) => {
  ...
  instance.use(EncryptMiddleware);

  instance.use(DecryptMiddleware);

  instance.use(AttachmentMiddleware);
