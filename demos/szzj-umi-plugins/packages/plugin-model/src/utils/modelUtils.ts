...
import { glob, winPath, fsExtra } from '@umijs/utils';
...
export class Model {
  ...
  findDeps(sort: object) {
    const content = fsExtra.readFileSync(this.file, 'utf-8');
...
export class ModelUtils {
  ...
  getAllModels(opts: { sort?: object; extraModels: string[]; excludeModels?: string[] }) {
    // reset count
    this.count = 1;
    const models = [
      ...this.getModels({
      ...
      .filter((file: string) => {
        if (opts?.excludeModels?.length) {
          const exclude = opts?.excludeModels.map((path) => {
            return winPath(join(this.api.paths.absSrcPath), path);
          });

          return exclude?.includes(file) ? false : true;
        }
    ...
    if (opts.sort) {
      ...
      models.sort(
        (a, b) => namespaces.indexOf(a.namespace) - namespaces.indexOf(b.namespace),
      );
    }
    return models;
  }
